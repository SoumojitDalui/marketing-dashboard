...
function Overview() {
  ...
  const funnel = platforms.reduce((all, [, platform]) => ({ impressions: all.impressions + metricNumber(platform.totals, 'impressions'), clicks: all.clicks + metricNumber(platform.totals, 'clicks'), conversions: all.conversions + metricNumber(platform.totals, 'conversions') }), { impressions: 0, clicks: 0, conversions: 0 });
  return <Layout><S.PageHead><div><S.Eyebrow>Marketing performance</S.Eyebrow><S.Heading>Your channel pulse</S.Heading><S.Subhead>30-day rolling overview across paid media.</S.Subhead></div></S.PageHead>{error ? <S.ErrorBox>{error}</S.ErrorBox> : !data ? <S.Loader>Loading channel performance…</S.Loader> : <><S.Panel style={{marginBottom:18}}><S.PanelTitle>Campaign funnel</S.PanelTitle><S.PanelHint>How paid reach progressed through the conversion journey.</S.PanelHint><S.Funnel><S.FunnelStep><S.Label>Impressions</S.Label><S.FunnelNumber>{fmt.format(funnel.impressions)}</S.FunnelNumber><S.FunnelRate>Top of funnel</S.FunnelRate></S.FunnelStep><S.FunnelStep><S.Label>Clicks</S.Label><S.FunnelNumber>{fmt.format(funnel.clicks)}</S.FunnelNumber><S.FunnelRate>{percent(funnel.clicks, funnel.impressions).toFixed(2)}% click-through rate</S.FunnelRate></S.FunnelStep><S.FunnelStep><S.Label>Conversions</S.Label><S.FunnelNumber>{fmt.format(funnel.conversions)}</S.FunnelNumber><S.FunnelRate>{percent(funnel.conversions, funnel.clicks).toFixed(2)}% click-to-conversion rate</S.FunnelRate></S.FunnelStep></S.Funnel></S.Panel><S.Grid>{platforms.map(([p, d]) => <NavLink key={p} to={PLATFORM[p].path} style={{ color: 'inherit', textDecoration: 'none' }}><PlatformCard platform={p} data={d} /></NavLink>)}</S.Grid><S.Panel style={{marginTop:18}}><S.PanelTitle>Network comparison</S.PanelTitle><S.PanelHint>Rank each platform on reach, outcomes, and efficiency.</S.PanelHint><S.TableWrap><S.ComparisonTable><thead><tr><th>Platform</th><th>Spend</th><th>Conversions</th><th>CPA</th><th>CTR</th><th>CPM</th></tr></thead><tbody>{platforms.map(([platform, item]) => { const totals = item.totals; const impressions = metricNumber(totals, 'impressions'), clicks = metricNumber(totals, 'clicks'), spend = metricNumber(totals, 'spend'), conversions = metricNumber(totals, 'conversions'); return <tr key={platform}><td><S.PlatformTag $color={PLATFORM[platform].color}>{PLATFORM[platform].name}</S.PlatformTag></td><td>{money.format(spend)}</td><td>{fmt.format(conversions)}</td><td>{money.format(cpa(spend, conversions))}</td><td>{percent(clicks, impressions).toFixed(2)}%</td><td>{money.format((spend / impressions) * 1000)}</td></tr>; })}</tbody></S.ComparisonTable></S.TableWrap></S.Panel><S.Footnote>Compare detail performance with the previous, equal-length period. Values may vary slightly between requests because this API generates synthetic data.</S.Footnote></>}</Layout>;
}
    ...
    {error ? <S.ErrorBox>{error}</S.ErrorBox> : loading || !data ? <S.Loader>Loading live network insights…</S.Loader> : <>
      <S.DetailGrid>
        <S.Panel><S.PanelTitle>Daily conversions & spend</S.PanelTitle>
          <div style={{height:260,marginTop:10}}><ResponsiveContainer width="100%" height="100%"><ComposedChart data={trend}><defs><linearGradient id="conversion-area" x1="0" x2="0" y1="0" y2="1"><stop offset="0%" stopColor={PLATFORM[active].color} stopOpacity={.3}/><stop offset="100%" stopColor={PLATFORM[active].color} stopOpacity={.01}/></linearGradient></defs><CartesianGrid vertical={false} stroke="#eef0f4"/><XAxis dataKey="date" tickFormatter={v=>displayDate(String(v))} tick={{fontSize:11,fill:'#858a9a'}} axisLine={false} tickLine={false} minTickGap={25}/><YAxis yAxisId="conversions" tickFormatter={v=>compact(Number(v))} tick={{fontSize:11,fill:'#858a9a'}} axisLine={false} tickLine={false} width={38}/><YAxis yAxisId="spend" orientation="right" tickFormatter={v=>`$${compact(Number(v))}`} tick={{fontSize:11,fill:'#858a9a'}} axisLine={false} tickLine={false} width={44}/><Tooltip labelFormatter={v=>displayDate(String(v))} formatter={(v,name) => [name === 'Spend' ? money.format(Number(v)) : fmt.format(Number(v)), name]} contentStyle={{borderRadius:10,border:'1px solid #e5e6ed',boxShadow:'0 6px 20px #292a3c16'}}/><Legend verticalAlign="top" align="right" iconType="circle" wrapperStyle={{fontSize:12,paddingBottom:12}}/><Area yAxisId="conversions" type="monotone" name="Conversions" dataKey="conversions" stroke={PLATFORM[active].color} strokeWidth={2.5} fill="url(#conversion-area)"/><Line yAxisId="spend" type="monotone" name="Spend" dataKey="spend" stroke="#f59e0b" strokeWidth={2.5} dot={false}/></ComposedChart></ResponsiveContainer></div>
        </S.Panel>
        <S.Panel><S.PanelTitle>Period at a glance</S.PanelTitle><S.PanelHint>Change versus {rangeLabel(data.previousPeriod.startDate,data.previousPeriod.endDate)}</S.PanelHint><S.MetricGrid>{overviewMetrics.map(key=>{const d=delta(totals[key] ?? 0, previous[key] ?? 0, ['cpc','cpm'].includes(key)); return <S.MetricCard key={key}><S.Label>{key === 'ctr' ? 'CTR' : key.replace(/([A-Z])/g,' $1')}</S.Label><strong style={{fontSize:16,display:'block',margin:'4px 0 7px'}}>{value(totals[key] ?? 0,key)}</strong><S.Delta $tone={key==='spend' ? 'neutral' : d.tone}>{d.text}</S.Delta></S.MetricCard>})}</S.MetricGrid></S.Panel>
      </S.DetailGrid>
      <S.Divider/>
      <S.Panel><S.PanelTitle>Daily signals</S.PanelTitle><S.PanelHint>Best and worst days in the selected period.</S.PanelHint><S.InsightGrid>{bestConversionDay && <S.InsightCard><S.Label>Best conversion day</S.Label><strong>{displayDate(String(bestConversionDay.date))}</strong><span>{fmt.format(metricNumber(bestConversionDay, 'conversions'))} conversions</span></S.InsightCard>}{lowestConversionDay && <S.InsightCard><S.Label>Lowest conversion day</S.Label><strong>{displayDate(String(lowestConversionDay.date))}</strong><span>{fmt.format(metricNumber(lowestConversionDay, 'conversions'))} conversions</span></S.InsightCard>}{costliestDay && <S.InsightCard><S.Label>Highest cost per conversion</S.Label><strong>{displayDate(String(costliestDay.date))}</strong><span>{money.format(cpa(metricNumber(costliestDay, 'spend'), metricNumber(costliestDay, 'conversions')))} per conversion</span></S.InsightCard>}</S.InsightGrid></S.Panel>
      <S.Divider/>
