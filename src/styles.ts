...
export const FillMetricGrid = styled(MetricGrid)`flex:1; grid-template-rows:repeat(2,minmax(0,1fr)); @media(max-width:540px){grid-template-rows:none;}`;
...
export const TableWrap = styled.div`overflow-x:auto; margin-top:15px; @media(max-width:700px){overflow:visible;}`;
export const ComparisonTable = styled.table`border-collapse:collapse; width:100%; min-width:630px; font-size:13px; th{text-align:right; color:#858a9a; font-weight:700; font-size:11px; text-transform:uppercase; letter-spacing:.04em; padding:0 10px 10px; &:first-child{text-align:left;padding-left:0;}} td{text-align:right; padding:13px 10px; border-top:1px solid #edf0f4; color:#36394a; font-weight:700; &:first-child{text-align:left;padding-left:0;}} @media(max-width:700px){min-width:0; thead{display:none;} tbody{display:grid;gap:10px;} tr{display:grid;grid-template-columns:repeat(2,minmax(0,1fr));padding:6px 12px;border:1px solid #ececf5;border-radius:11px;background:#fcfcfe;} td{display:flex;align-items:center;justify-content:space-between;gap:8px;padding:10px 0;border:0;text-align:right;&:first-child{grid-column:1/-1;padding-left:0;border-bottom:1px solid #e9ebf2;} &::before{content:attr(data-label);color:#858a9a;font-weight:650;font-size:11px;text-align:left;}} td:first-child::before{display:none;}}`;
...
export const SignalLabel = styled(Label)<{ $color: string }>`display:flex; align-items:center; gap:7px; &:before{content:'';width:8px;height:8px;border-radius:50%;background:${p=>p.$color};box-shadow:0 0 0 3px color-mix(in srgb, ${p=>p.$color} 14%, transparent);}`;
export const ShareGrid = styled.div`display:grid; grid-template-columns:repeat(4,1fr); gap:11px; margin-top:16px; @media(max-width:900px){grid-template-columns:repeat(2,1fr)} @media(max-width:560px){grid-template-columns:1fr}`;
export const ShareCard = styled.div`padding:15px; border:1px solid #ececf5; border-radius:11px; background:#fcfcfe;`;
export const ShareMetric = styled.div`margin-top:11px; font-size:11px; color:#83889a; strong{float:right;color:#353849;font-size:12px;} div{clear:both;height:5px;background:#e9ebf2;border-radius:999px;margin-top:5px;overflow:hidden;} i{display:block;height:100%;border-radius:inherit;background:#5f5aab;}`;
